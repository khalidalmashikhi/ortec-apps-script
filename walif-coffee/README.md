# Walif Coffee — وليف كوفي · النظام المالي والتشغيلي
...
## نسخة إعادة البيع (White-label)

الكود واحد لكل الزبائن؛ الهوية (الاسم بالعربية والإنجليزية، كلمة الشعار، السطر الصغير، اللونان، العملة، أو صورة شعار) تُضبط من **الإعدادات → هوية المحل** داخل حساب الإدارة، وتنعكس على صفحة الدخول والشريط العلوي وتقارير PDF والبريد. الافتراضي العام "Cafe Books / دفاتر الكافيه"، ونسخة وليف كوفي تُصنَّف تلقائيًا بهويتها لأن معرّف ملفها محفوظ في `Config.gs`.

**القالب الرئيسي (في حساب khalid98115159@gmail.com):**
https://docs.google.com/spreadsheets/d/1Dsu0PZW_TvavfA2MX7RU-JDnnljtqENdQQt_68LpqQ4/edit — لا يُستخدم مباشرة، بل يُنسخ.

**تثبيت نسخة لزبون جديد (5 دقائق):**
1. افتح القالب الرئيسي → File → **Make a copy** → غيّر الاسم (مثل "Bean Bar Accounting") واختر حساب/مجلد الزبون (أو شارك القالب مع الزبون ليعمل النسخة في حسابه هو؛ الكود يُنسخ مع الملف).
2. في النسخة: Extensions → Apps Script → **Deploy → New deployment** → النوع Web app → Execute as **Me** → Who has access **Anyone** → Deploy → انسخ رابط `/exec`.
3. افتح الرابط ووافق على الصلاحيات مرة واحدة. تُبنى الأوراق والمجلدات (باسم المحل) والحسابات تلقائيًا.
4. ادخل `admin` / `2026` → الإعدادات → **هوية المحل**: الاسم، كلمة الشعار، الألوان، العملة → حفظ. ثم البريد وساعة التقرير → حفظ. ثم غيّر كلمتي المرور.
5. سلّم الزبون الرابط وبيانات الدخول.

**تحديث كل النسخ لاحقًا:** ادفع الكود إلى مشروع القالب (`.clasp.template.json`) ثم لكل زبون `clasp push` بمعرّف مشروعه، أو أعطه المثبّت `installer/` ليشغّله في مشروعه. البيانات والهوية لا تتأثر بالتحديث.
...
- **الحركة النقدية**: صافي المبيعات − المشتريات المدفوعة − المصروفات المدفوعة − الرواتب المدفوعة − الإيجار المدفوع − الكاش المسحوب (للمصاريف/أخرى) + ما دُفع بطريقة «كاش مسحوب».
- **رصيد الحساب البنكي** (الإعدادات ← الحساب البنكي): الرصيد الافتتاحي + الحركة النقدية من تاريخ الرصيد الافتتاحي حتى اليوم. يظهر في لوحة التحكم والتقرير اليومي.
- **سحب الكاش** (ورقة `Cash_Withdrawals`): تاريخ، مبلغ، الوجهة (إيداع في البنك / مصاريف / أخرى)، بيان، صورة. الإيداع في البنك تحويل داخلي لا يغيّر الرصيد؛ المسحوب للمصاريف يُخصم مرة واحدة، وأي مصروف يُدفع منه يُسجَّل بطريقة الدفع «كاش مسحوب» فيُحتسب في الربح ولا يُخصم من النقد ثانية.
