function createTask(input, sessionToken){
  const user=requireCapability_('tasks', sessionToken); input=input||{};
...
/** Public entry point: authenticate before exposing the task list. */
function listTasks(status, sessionToken){
  requireCapability_('tasks', sessionToken);
  return listTasks_(status);
}

function listTasks_(status){return sheetToObjects_(ORTEC.SHEETS.TASKS).filter(function(t){return !status||status==='ALL'||(status==='OPEN'?t.status!=='COMPLETED':t.status===status);}).slice(-500).reverse();}
function completeTask(taskId, sessionToken){
  const user=requireCapability_('tasks', sessionToken);
  const old=findBy_(ORTEC.SHEETS.TASKS,'task_id',taskId);if(!old)throw new Error('المهمة غير موجودة.');
  // Closing someone else's task requires task management rights.
  const isAssignee=String(old.assigned_to||'')!==''&&String(old.assigned_to)===String(user.email||user.username);
  if(!isAssignee&&capabilitiesForRole_(user.role).indexOf('tasks.manage')===-1)throw new Error('لا يمكنك إغلاق مهمة غير مسندة إليك.');const patch={status:'COMPLETED',completed_at:nowIso_()};updateById_(ORTEC.SHEETS.TASKS,'task_id',taskId,patch);audit_('TASK',taskId,'COMPLETE',old,Object.assign({},old,patch));return {ok:true};}
function sendTaskReminders(event, sessionToken){
  requireScheduledOrCapability_(event, sessionToken, 'tasks.manage');
  const today=today_(); const tasks=listTasks_('OPEN').filter(function(t){return t.due_date && normalizeDate_(t.due_date)<=today;});
